...
RUN apt update && apt upgrade -y --no-install-recommends && apt install libgflags-dev python3-pip libopencv-dev openssl libssl-dev libopencv-contrib-dev libarpack++2-dev libarpack2-dev libsuperlu-dev wget curl git nano build-essential cmake libgflags-dev libunwind-dev libeigen3-dev libgflags-dev libopencv-dev gcc-9 g++-9 -y --no-install-recommends && apt clean && apt autoclean
...
RUN bash -c "cd DeepLSD && cd third_party/progressive-x/graph-cut-ransac/build; cmake ..; make -j8" && bash -c "cd DeepLSD && cd third_party/progressive-x/build; cmake ..; make -j8;" && bash -c "cd DeepLSD && cd third_party/progressive-x/graph-cut-ransac/; rm -rf build" && bash -c "cd DeepLSD && cd third_party/progressive-x/; rm -rf build"
