...
RUN apt update && apt upgrade -y --no-install-recommends && apt install python3-distutils software-properties-common libceres-dev lsb-core libatlas-base-dev liblapack-dev libblas-dev ninja-build python3.8-dev libgflags-dev libopencv-dev openssl libssl-dev libopencv-contrib-dev libarpack++2-dev libarpack2-dev libsuperlu-dev wget curl git nano build-essential cmake libgflags-dev libunwind-dev libeigen3-dev libgflags-dev libopencv-dev -y --no-install-recommends &&  add-apt-repository ppa:ubuntu-toolchain-r/test && apt install gcc-9 g++-9 -y --no-install-recommends && apt clean && apt autoclean
