...
RUN source ~/.bashrc && mkdir -p ~/catkin_plvins/src && cd ~/catkin_plvins/ && catkin_make && source devel/setup.bash && cd src && git clone https://github.com/cnqiangfu/PL-VINS.git && cd PL-VINS/feature_tracker && rm CMakeLists.txt && wget https://raw.githubusercontent.com/Jiu-xiao/DeepLSD-Docker/refs/heads/main/CMakeLists.txt && cd ~/catkin_plvins && catkin_make
