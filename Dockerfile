...
RUN apt update && apt upgrade -y --no-install-recommends && apt install libceres-dev libatlas-base-dev liblapack-dev libblas-dev ninja-build python3.8-dev libgflags-dev python3-pip libopencv-dev openssl libssl-dev libopencv-contrib-dev libarpack++2-dev libarpack2-dev libsuperlu-dev wget curl git nano build-essential cmake libgflags-dev libunwind-dev libeigen3-dev libgflags-dev libopencv-dev gcc-9 g++-9 -y --no-install-recommends && apt clean && apt autoclean
...
RUN pip install pybind11 && git clone --recursive https://github.com/Jiu-xiao/DeepLSD.git --depth=1 && cd DeepLSD && git submodule init && git submodule update && pip install -r requirements.txt && pip install -r requirements.txt && rm -rf ~/.cache/pip
